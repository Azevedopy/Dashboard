...
export function GenierTable({ projects, isLoading, onDelete }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        try {
          return format(parseISO(row.getValue("data_inicio")), "dd/MM/yyyy", { locale: ptBR })
        } catch (error) {
          console.error("Error formatting date:", error)
          return row.getValue("data_inicio")
        }
      },
    },
    {
      ...
      cell: ({ row }) => {
        try {
          return format(parseISO(row.getValue("data_termino")), "dd/MM/yyyy", { locale: ptBR })
        } catch (error) {
          console.error("Error formatting date:", error)
          return row.getValue("data_termino")
        }
      },
    },
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => router.push(`/genier/projetos/${project.id}`)}>
                <Eye className="mr-2 h-4 w-4" />
                <span>Visualizar</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => router.push(`/genier/projetos/${project.id}/editar`)}>
