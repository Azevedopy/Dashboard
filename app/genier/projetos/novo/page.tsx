...
import { createGenierProject } from "@/lib/genier-service"
import { useToast } from "@/components/ui/use-toast"

export default function NovoProjetoGenierPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validação básica
    if (!formData.cliente || !formData.tipo || !formData.data_inicio || !formData.data_termino || !formData.porte) {
      toast({
    ...
    try {
      const result = await createGenierProject(formData)
    ...
    <div className="flex flex-col h-full">
      <div className="p-6 bg-[#0056D6] text-white">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Novo Projeto Genier</h1>
            <p className="text-sm text-white/90">Cadastre um novo projeto Genier</p>
          </div>
        </div>
      </div>
