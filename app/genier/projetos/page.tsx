...
import { GenierStats } from "@/components/genier/genier-stats"
import { GenierTable } from "@/components/genier/genier-table"
import {
  getGenierProjects,
  getGenierStats,
...
export default function GenierProjetosPage() {
  ...
  const handleReset = async () => {
    ...
    setIsLoading(true)
    try {
      const projectsData = await getGenierProjects()
      setProjects(projectsData)
    } catch (error) {
  ...
  const handleExport = () => {
    ...
    const result = exportToCSV(projects, "projetos_genier")
    toast({
  ...
  const handleDelete = async (id) => {
    if (confirm("Tem certeza que deseja excluir este projeto? Esta ação não pode ser desfeita.")) {
      try {
        const success = await deleteGenierProject(id)
        if (success) {
          toast({
            title: "Sucesso",
          ...
          const updatedProjects = await getGenierProjects()
          setProjects(updatedProjects)
          // Atualizar estatísticas
          const updatedStats = await getGenierStats()
    ...
    <div className="flex flex-col h-full">
      <div className="p-6 border-b">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Projetos Genier</h1>
            <p className="text-sm text-muted-foreground">Gerencie e visualize todos os projetos Genier</p>
          </div>
          <Button onClick={() => router.push("/genier/projetos/novo")}>
      ...
      <div className="p-6 space-y-6">
        <GenierStats stats={stats} isLoading={isLoading} />
        ...
        <Tabs defaultValue="todos">
          ...
          <TabsContent value="todos" className="mt-6">
            <GenierTable projects={projects} isLoading={isLoading} onDelete={handleDelete} />
          </TabsContent>
