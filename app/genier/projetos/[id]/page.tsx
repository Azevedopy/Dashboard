...
import { getGenierProjectById, deleteGenierProject } from "@/lib/genier-service"
import { useToast } from "@/components/ui/use-toast"
import { formatCurrency } from "@/lib/utils"

export default function DetalhesProjetoGenierPage({ params }) {
  ...
  useEffect(() => {
    async function fetchProject() {
      try {
        const projectData = await getGenierProjectById(params.id)
        if (projectData) {
  ...
  const handleDelete = async () => {
    if (confirm("Tem certeza que deseja excluir este projeto? Esta ação não pode ser desfeita.")) {
      try {
        const success = await deleteGenierProject(params.id)
        if (success) {
          toast({
            title: "Sucesso",
      ...
      <div className="p-6 border-b">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold">{project.cliente}</h1>
              <div className="flex items-center gap-2">
                <p className="text-sm text-muted-foreground">{project.tipo}</p>
                {project.status && getStatusBadge(project.status)}
              </div>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push(`/genier/projetos/${params.id}/editar`)}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center">
                ...
                <span>
                  {format(parseISO(project.data_inicio), "dd/MM/yyyy", { locale: ptBR })} a{" "}
                  {format(parseISO(project.data_termino), "dd/MM/yyyy", { locale: ptBR })}
                </span>
              </div>
              <div className="flex items-center">
                <Clock className="h-4 w-4 mr-2 text-muted-foreground" />
                <span className="text-sm font-medium mr-2">Duração:</span>
                <span>{project.tempo_dias} dias</span>
              </div>
              <div className="flex items-center">
                <DollarSign className="h-4 w-4 mr-2 text-muted-foreground" />
                <span className="text-sm font-medium mr-2">Valor:</span>
                <span>{formatCurrency(project.valor_consultoria)}</span>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{formatCurrency(project.valor_consultoria)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Valor do Bônus</p>
                  <p className="text-lg font-semibold">{formatCurrency(project.valor_bonus)}</p>
                </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p>{project.prazo_atingido ? "Sim" : "Não"}</p>
              </div>
            </div>
