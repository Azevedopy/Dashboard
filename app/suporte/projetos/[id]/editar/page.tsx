...
import { getSupportProjectById, updateSupportProject } from "@/lib/support-service"
import { useToast } from "@/components/ui/use-toast"

export default function EditarProjetoSuportePage({ params }) {
  ...
  useEffect(() => {
    async function fetchProject() {
      try {
        const project = await getSupportProjectById(params.id)
        if (project) {
          setFormData({
            ...
            tempo_dias: project.tempo_dias || 0,
            porte: project.porte || "",
            porte_detalhado: project.porte_detalhado || "",
            valor_consultoria: project.valor_consultoria || 0,
            valor_bonus: project.valor_bonus || 0,
            consultor: project.consultor || "",
            status: project.status || "",
            prazo_atingido: project.prazo_atingido || false,
          })

          // Configurar datas para os calendários
          if (project.data_inicio) {
            setStartDate(parseISO(project.data_inicio))
          }
          if (project.data_termino) {
            setEndDate(parseISO(project.data_termino))
          }
        } else {
          toast({
            title: "Erro",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await updateSupportProject(params.id, formData)
      ...
      if (result) {
        ...
        router.push(`/suporte/projetos/${params.id}`)
      } else {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-6">
          <div className="h-96 flex items-center justify-center">
            <div className="animate-pulse">Carregando dados do projeto...</div>
          </div>
        </div>
      </div>
    )
